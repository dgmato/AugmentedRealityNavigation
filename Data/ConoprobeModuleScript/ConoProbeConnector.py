...
class ConoProbeConnector(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.contributors = ["Mikael Brudfors (Laboratorio de Imagen Medica, Hospital Gregorio Maranon - http://image.hggm.es/)"] 
...
class ConoProbeConnectorWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.minSpinBox.setMinimum(-200)
    self.minSpinBox.setMaximum(200)
    ...
    self.maxSpinBox.setMinimum(-200)
    self.maxSpinBox.setMaximum(200)
    self.maxSpinBox.setSingleStep(0.1)
    self.maxSpinBox.setValue(20.0)
...
class ConoProbeConnectorLogic(ScriptedLoadableModuleLogic):
  ...
  def addUpdateObserver(self):
    if self.outputObserverTag == -1:
      self.outputObserverTag = self.parametersToMeasurerTransformNode.AddObserver('ModifiedEvent', self.updateSceneCallback)
